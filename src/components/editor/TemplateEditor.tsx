...
import { ParticleCanvas, CanvasLoader } from '@/components/canvas/ParticleCanvas';
  ...
  return (
    <div className="w-full h-full flex bg-slate-900">
      ...
      <div className="w-80 bg-slate-800 border-r border-slate-700 p-4 overflow-y-auto">
        <div className="space-y-4">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-xs text-purple-400">{(particleCount || 0).toLocaleString()}</span>
                </div>
                <input 
                  type="range" 
                  min="10" 
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-xs text-purple-400">{(animationSpeed || 1.0).toFixed(1)}x</span>
                </div>
                <input 
                  type="range" 
                  min="0.1" 
              ...
              <div className="space-y-2">
                ...
                >
                  ...
                  <option value="flow">🎭 Fluid Motion</option>
                </select>
              </div>
            </div>
          </div>
          ...
          <div className="pt-4 border-t border-slate-700">
            <h3 className="text-md font-medium text-white mb-2">System Status</h3>
            <p className="text-sm text-gray-400 mb-3">
              Real-time particle dance system
            </p>
            <div className="bg-slate-700 p-3 rounded space-y-1">
              <div className="text-xs text-gray-300">Status: {isPlaying ? '▶️ Playing' : '⏸️ Ready'}</div>
              <div className="text-xs text-gray-300">Particles: {(particleCount || 0).toLocaleString()}</div>
              <div className="text-xs text-gray-300">FPS: {(fps || 60).toFixed(0)}</div>
              <div className="text-xs text-gray-300">Pattern: {patternType}</div>
            </div>
          </div>
          
          <div className="pt-4 border-t border-slate-700">
            <h3 className="text-md font-medium text-white mb-2">Dance Videos</h3>
            <div className="space-y-2 text-xs text-gray-400">
              <div>🎬 Video 1: Contemporary Flow</div>
              <div>🎬 Video 2: Rhythmic Beats</div>
              <div className="text-purple-400">Patterns inspired by @onno_quist</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col">
        ...
        {/* Timeline Controls */}
        <div className="h-20 bg-slate-800 border-t border-slate-700 p-3">
